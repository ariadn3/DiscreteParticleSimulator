...
__host__ void resolveValidCollisions(collision_t*, int*, float, float);
...
__constant__ float l, r;
__constant__ float minPosMargin, maxPosMargin;
__constant__ float maxPos;
...
__host__ int main(int argc, char** argv) {
    ...
    float minMargin = hostR + EDGE_TOLERANCE;
    float maxMargin = hostL - hostR - EDGE_TOLERANCE;
    float max = hostL - hostR;
    ...
    allocStatus = cudaMallocManaged((void**) &cs, hostN * hostN * sizeof(collision_t) / 2);
...
__global__ void checkWallCollision() {
    ...
    float x_time = NO_COLLISION;
    float y_time = NO_COLLISION;
    ...
    float wall_time = x_time < y_time ? x_time : y_time;
...
__global__ void checkCollision() {
    ...
    float dX = q.x - p.x;
    float dY = q.y - p.y;
    float dVx = q.v_x - p.v_x;
    float dVy = q.v_y - p.v_y;
    ...
    // that are dT < 0
    float dT = (-B - sqrt(discriminant)) / 2 / A;
...
__global__ void settleCollision() {
    ...
    // with the nearest wall and take that time
    float time_ax = 1 - time;
    float time_ay = 1 - time;
...
__global__ void updateParticles() {
    ...
    // printf("Thread %d updating particle %d\n", index, curParticle->id);
