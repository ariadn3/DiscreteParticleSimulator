...
int oneDChunkSize, twoDChunkSize;
...
__host__ int main(int argc, char** argv) {
    if (argc >= 2) {
        oneDChunkSize = atoi(argv([1]));
    } else {
        oneDChunkSize = 32;
    }
    if (argc >= 3) {
        twoDChunkSize = atoi(argv([2]));
    } else {
        twoDChunkSize = 32;
    }
...
__host__ void simulate() {
    ...
    int pwChunkSize = oneDChunkSize;
    dim3 pwGrid((hostN + pwChunkSize - 1) / pwChunkSize);
    dim3 pwBlock(chunkSize);

    int ppChunkSize = twoDChunkSize;
    dim3 ppGrid((hostN + 1) / 2, (hostN + ppChunkSize - 1) / ppChunkSize);
    dim3 ppBlock(pwChunkSize);

    int resolveChunkSize = oneDChunkSize;

    int updateChunkSize = oneDChunkSize;
