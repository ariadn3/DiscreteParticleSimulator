...
void simulate(void);
float checkWallCollision(float, float, particle_t*);
float checkCollision(float, particle_t*, particle_t*);
        ...
        *numCollisions = 0;
        ...
        #pragma omp parallel
        {
            ...
            for (int p = 0; p < n; p++) {
                ...
                float wallTime = checkWallCollision(r, l, ps[p]);
                if (DEBUG_LEVEL > 2)
                ...
                for (int q = p + 1; q < n; q++) {
                    ...
                    float time = checkCollision(r, ps[p], ps[q]);
...
float checkCollision(float r, particle_t* p, particle_t* q) {
